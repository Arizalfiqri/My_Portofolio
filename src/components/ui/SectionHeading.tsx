...
export default function SectionHeading({ title, subtitle, className = '' }: { title: string, subtitle?: string, className?: string }) {
  return (
    <div className={`text-center max-w-2xl mx-auto mb-16 md:mb-20 px-4 sm:px-0 ${className}`}>
      ...
      >
        <h2 className="heading-section font-bold font-outfit mb-4 text-foreground dark:text-dark-text relative inline-block">
          {title}
        ...
        {subtitle && (
          <p className="text-foreground/50 dark:text-dark-text/50 text-body mt-6 leading-relaxed">{subtitle}</p>
        )}
      </motion.div>
    </div>
  );
