...
export default function AchievementCard({ title, issuer, date, description }: AchievementProps) {
  return (
    <motion.div
      ...
      className="warm-card p-6 h-full flex flex-col cursor-hover group"
    >
      <div className="w-11 h-11 rounded-xl bg-primary/8 flex items-center justify-center mb-5 group-hover:bg-primary/15 group-hover:scale-110 transition-all duration-300">
        <Trophy className="w-5 h-5 text-primary" />
      </div>
      
      <h4 className="text-lg font-bold font-outfit text-foreground dark:text-dark-text mb-2">{title}</h4>
